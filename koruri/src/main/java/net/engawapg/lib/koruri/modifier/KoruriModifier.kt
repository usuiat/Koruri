...
/**
 * An ordered, immutable collection of [modifier elements][KoruriModifier.Element] that decorate or add
 ...
 *
 * Modifier implementations should offer a fluent factory extension function on [KoruriModifier] for
 ...
 *
 * Composables that accept a [KoruriModifier] as a parameter to be applied to the whole component
 * represented by the composable function should name the parameter `modifier` and
 * assign the parameter a default value of [KoruriModifier]. It should appear as the first
 ...
 * composable function's implementation, keeping [KoruriModifier] as the default parameter value.
 *
 ...
 */
@Stable
public interface KoruriModifier {
    ...
    /**
     * Returns `true` if [predicate] returns true for any [Element] in this [KoruriModifier].
     */
    public fun any(predicate: (Element) -> Boolean): Boolean

    /**
     * Returns `true` if [predicate] returns true for all [Element]s in this [KoruriModifier] or if
     * this [KoruriModifier] contains no [Element]s.
     */
     ...
     *
     * Returns a [KoruriModifier] representing this modifier followed by [other] in sequence.
     */
    public infix fun then(other: KoruriModifier): KoruriModifier =
        if (other === KoruriModifier) this else CombinedKoruriModifier(this, other)

    /**
     * A single element contained within a [KoruriModifier] chain.
     */
    public interface Element : KoruriModifier {
    ...
    /**
     * The companion object `Modifier` is the empty, default, or starter [KoruriModifier]
     * that contains no [elements][Element]. Use it to create a new [KoruriModifier] using
     * modifier extension factory functions or as the default value for [KoruriModifier] parameters.
     */
    // The companion object implements `Modifier` so that it may be used as the start of a
    // modifier extension factory expression.
    public companion object Companion : KoruriModifier {
        override fun <R> foldIn(initial: R, operation: (R, Element) -> R): R = initial
        override fun <R> foldOut(initial: R, operation: (Element, R) -> R): R = initial
        override fun any(predicate: (Element) -> Boolean): Boolean = false
        override fun all(predicate: (Element) -> Boolean): Boolean = true
        override infix fun then(other: KoruriModifier): KoruriModifier = other
        override fun toString(): String = "Modifier"
    }
}

/**
 * A node in a [KoruriModifier] chain. A CombinedModifier always contains at least two elements;
 * a Modifier [outer] that wraps around the Modifier [inner].
 */
public class CombinedKoruriModifier(
    internal val outer: KoruriModifier,
    internal val inner: KoruriModifier,
) : KoruriModifier {
    override fun <R> foldIn(initial: R, operation: (R, KoruriModifier.Element) -> R): R =
        inner.foldIn(outer.foldIn(initial, operation), operation)

    override fun <R> foldOut(initial: R, operation: (KoruriModifier.Element, R) -> R): R =
        outer.foldOut(inner.foldOut(initial, operation), operation)

    override fun any(predicate: (KoruriModifier.Element) -> Boolean): Boolean =
        outer.any(predicate) || inner.any(predicate)

    override fun all(predicate: (KoruriModifier.Element) -> Boolean): Boolean =
        outer.all(predicate) && inner.all(predicate)

    override fun equals(other: Any?): Boolean =
        other is CombinedKoruriModifier && outer == other.outer && inner == other.inner
